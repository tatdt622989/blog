...
function addTaxonomyDirectoriesToSitemap(xml, localeHomeUrl) {
  const entries = ['tags/', 'categories/'].map(directory => {
    const url = new URL(directory, localeHomeUrl).toString();
    return `<loc>${escapeXml(url)}</loc>`;
  }).filter(location => !xml.includes(location));

  if (!entries.length) return xml;
  if (!/<\/urlset>\s*$/.test(xml)) throw new Error('Invalid localized sitemap: missing </urlset>');

  const directoryUrls = entries.map(location => `  <url>\n    ${location}\n  </url>`).join('\n');
  return xml.replace(/<\/urlset>\s*$/, `${directoryUrls}\n</urlset>\n`);
}
