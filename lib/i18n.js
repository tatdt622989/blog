...
function alternateLinks(page = {}, config = {}) {
  ...
  if (isHomepage(page) || ['tags', 'categories'].includes(page.taxonomy_index)) {
    const directory = page.taxonomy_index ? `${page.taxonomy_index}/` : '';
    const links = Object.keys(localePaths).map(locale => ({
      hreflang: locale,
      href: absoluteSiteUrl(localeHomePath(locale, config) + directory, config),
    }));
...
function languageSwitchTarget(page = {}, targetLocale, config = {}) {
  ...
  if (['tags', 'categories'].includes(page.taxonomy_index)) {
    return localeHomePath(normalizedTarget, config) + page.taxonomy_index + '/';
  }
