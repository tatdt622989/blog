...
4. **封面圖可用 PixelLab MCP 生成**
   - 本專案已設定 PixelLab MCP server（見 `~/.Codex.json` 對應本專案的 `mcpServers.pixellab`）。
...
6. **Mermaid 流程圖繪製與排版規則（詳見 `.agents/skills/mermaid-flowcharts/SKILL.md`）**
   - **嚴禁使用 `subgraph` 容器**：避免 dagre 佈局在處理跨群組連線時將子圖左右拆分、導致標題與內部節點嚴重重疊疊字。
   - **嚴禁使用 `{...}` 巨大菱形節點**：避免橫向幾何擠壓破版，條件閘門改用圓角矩形或膠囊節點（如 `G1["PLAN GATE (規格閘門)"]`）。
...
## Co-Authored-By: Codex Opus 4.8 <noreply@anthropic.com> 不要寫這個在 commit