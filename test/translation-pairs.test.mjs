...
translations:
  zh-CN: /zh-cn/2023/03/05/wrong-simplified-path/
...
test('all Simplified Chinese posts satisfy the multilingual content and SEO contract', () => {
  const postsDir = path.join(projectRoot, 'source', 'zh-cn', '_posts');
  const postFiles = fs.readdirSync(postsDir)
    .filter(file => file.endsWith('.md'))
    .sort();

  assert.ok(postFiles.length > 0, 'at least one Simplified Chinese post');
  ...
  withFixture(
    {
      ...
      'source/zh-cn/_posts/2023-03-05-what-is-typescript.md': simplifiedChinesePost,
...
test('a translation key with no peer is rejected', () => {
  withFixture(
    ...
    root => {
      ...
      assert.ok(result.errors.some(error => error.includes('Missing en peer') || error.includes('Missing zh-CN peer')));
    },
  );
});
...
test('declared alternate paths must match the peer public path in both directions', () => {
  withFixture(
    ...
    root => {
      ...
      assert.ok(result.errors.some(error => error.includes('alternate path mismatch')));
    },
  );
});
