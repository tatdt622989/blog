...
### 任務目標與交付標準
- **角色設定**：銀髮、深紅瞳孔、黑色皮質斗篷、手握雙匕首。
- **動作要求**：低身拔刀衝刺的動態姿態，透視為微仰角，五指抓握匕首刀柄必須嚴格符合人體工學。
- **交付格式**：解析度 1536x1536、帶有透明背景（Alpha 通道）的 PNG 精靈圖，可直接匯入遊戲引擎。

### 五步管線搭建流程

#### 第一步：鎖定角色外貌（IP-Adapter 特徵注入）
在 ComfyUI 畫布中載入底模（如 Flux 或 SDXL），並串接 **IP-Adapter** 節點。將一張預先定稿的角色正面半身照餵入參考端口，並將權重設定為 **0.6**。這一步確立了角色的五官與髮型錨點，後續換動作時不會發生換臉漂移。

#### 第二步：鎖定空間動作（3D 姿態與 ControlNet）
不透過文字憑空猜測肢體。打開免費 3D 人偶工具，花費一分鐘拖曳出低身衝刺的骨架姿態，匯出 **OpenPose 骨骼圖**，並接入 **ControlNet (OpenPose)** 節點，權重設定為 **0.8**。這為模型施加了剛性物理枷鎖，確保四肢角度與刀柄方位分毫不差。

#### 第三步：快速產出粗胚並鎖定隨機種子（Low-Res Base）
提示詞僅保留核心要素（如 **silver hair, red eyes, leather cloak, dual daggers, dynamic combat dash**），解析度設定為 **1024x1024** 進行單次快速採樣。該步驟只需花費數秒驗證斗篷擺動幅度與整體光影。**一旦構圖符合預期，立即將隨機種子（Seed）由隨機切換為固定（Fixed）**，封存當前潛在特徵。

#### 第四步：自動切片修手修臉（ADetailer Pipeline）
...
#### 第五步：二次採樣長細節與一鍵透明去背（Latent Upscale & Rembg）
將局部精修後的潛在張量送入 **Latent Upscale** 節點，以 **1.5 倍** 放大並配合 **0.30** 的淺層降噪步數重新渲染，使斗篷皮革與金屬刀刃自然生長出高密度材質紋理。最後接入 **Rembg** 去背節點自動剔除背景雜色。

按下佇列生成按鈕後，整條管線全自動運轉，最終直接輸出符合遊戲交付標準的透明背景戰鬥立繪。
