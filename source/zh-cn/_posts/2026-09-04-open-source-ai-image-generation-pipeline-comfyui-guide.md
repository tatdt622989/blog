...
#### 第一步：锁定角色面貌（IP-Adapter 特征注入）
在 ComfyUI 画布中加载底模（如 Flux 或 SDXL），并串接 **IP-Adapter** 节点。将一张预先定稿的角色正面半身照喂入参考端口，并将权重设置为 **0.6**。这一步确立了角色的五官与发型锚点，后续变换动作时不会发生换脸漂移。

#### 第二步：锁定空间动作（3D 姿态与 ControlNet）
拒绝通过文字凭空猜测肢体。打开免费 3D 人偶工具，花几十秒拖曳出低身冲刺的骨架姿态，导出 **OpenPose 骨骼图**，并接入 **ControlNet (OpenPose)** 节点，权重设置为 **0.8**。这为模型施加了刚性物理锁，确保四肢角度与刀柄方位分毫不差。

#### 第三步：快速产出粗胚并锁定随机种子（Low-Res Base）
提示词仅保留核心要素（如 **silver hair, red eyes, leather cloak, dual daggers, dynamic combat dash**），分辨率设置为 **1024x1024** 进行单次快速采样。该步骤只需耗时数秒验证斗篷摆动幅度与整体光影。**一旦构图符合预期，立即将随机种子（Seed）由随机切换为固定（Fixed）**，封存当前潜空间特征。

#### 第四步：自动切片修手修脸（ADetailer Pipeline）
...
#### 第五步：二次采样长细节与一键透明去背（Latent Upscale & Rembg）
将局部精修后的潜空间张量送入 **Latent Upscale** 节点，以 **1.5 倍** 放大并配合 **0.30** 的浅层降噪步数重新渲染，使斗篷皮革与金属刀刃自然生长出高密度材质纹理。最后接入 **Rembg** 去背节点自动剔除背景杂色。

点击队列生成按钮后，整条管线全自动运转，最终直接输出符合游戏交付标准的透明背景战斗立绘。
